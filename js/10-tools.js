...
async function _trkStop() {
  ...
  // 있었는데, 그 패널을 걷으면서 함께 내렸다. 지금 저장하지 않으면
  // 사라지므로, 그 사실을 물음에 그대로 적는다.
  const asGpx = await uiConfirm(
    `항적 ${_trkPts.length}점을 기록했습니다.\n\n` +
    `지금 저장하지 않으면 이 기록은 사라집니다.`,
    { okText: '지금 GPX 저장', cancelText: '버리기' });
  if (asGpx) {
...
//
// ── 접기 · 펼치기 ────────────────────────────────────────────────
// 버튼과 글씨를 1.5 배로 키운 뒤로 줄이 지도 왼쪽을 넉넉히 가린다. 지형을
